...
from src.server.ScaffoldVI import ScaffoldVIServer      # 你的“m blocks, no averaging”版本
...
def evaluate_personalized_common_test(server, clients, testset, *, batch_size, device, loss_fn):
    """逐个客户端用其个性化 block 在公共测试集上评估，并对指标取平均。"""
    results = []
    for i, c in enumerate(clients):
        block_i = server.get_block(i)                      # CPU tensors
        c.set_broadcast(block_i, _zeros_like_state(block_i))  # c 全 0 即可
...
def run_scaffoldvi_personal_with_k(k_value, init_state, train_subsets, testset, device):
    ...
    owned_keys = list(base_model.state_dict().keys())  # 每个 client 的 block = 整个模型
    ...
    # 服务器（维护 m 个 block，不做跨客户平均）
    server = ScaffoldVIServer(base_model, clients, device=device)
    ...
    for r in range(1, TRAINING["rounds"] + 1):
        ...
        print(
            f"[k={k_value}, Round {r}] selected={stats['selected']} | "
            f"samples={stats['total_samples']} | eta={eta_r:.3f} | lambda={lam_r:.3f} | "
            f"gamma_g={stats['gamma_g']:.3f} | gamma_l={stats['gamma_l']:.3f}"
        )

        # 每 10 轮打印所有客户端的 block + ||c_i||（从客户端对象取）
        if r % 10 == 0:
            for idx in range(DATASET["num_clients"]):
                block = server.get_block(idx)  # CPU
                ci = clients[idx].ci           # dict of tensors (device)
                ci_norm = float(sum(v.detach().float().norm().item() for v in ci.values())) if ci is not None else 0.0
